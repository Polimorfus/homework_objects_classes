    ...
    def rate_lesson(self, lecturer, course, grade):
        ...
        for key in lecturer.grades.keys():
          for grad in list(lecturer.grades[key]):
            sum = sum + grad
            len += 1
        lecturer.average_rating = sum / len
    ...
    def rate_hw(self, student, course, grade):
        ...
        for key in student.grades.keys():
          for grad in list(student.grades[key]):
            sum = sum + grad
            len += 1
        student.average_rating = sum / len
...
first_student.finished_courses += ["С++"]
first_student.courses_in_progress += ["GIT"]
first_student.courses_in_progress += ["Python"]
second_student = Student("Иван", "Петров", "м")
second_student.courses_in_progress += ["Python"]
second_student.finished_courses += ["C++"]
first_lecturer = Lecturer("Сергей", "Васильев")
first_lecturer.courses_attached += ["Python"]
first_lecturer.courses_attached += ["GIT"]
second_lecturer = Lecturer("Александр", "Пушкин")
second_lecturer.courses_attached += ["GIT"]
first_reviewer = Reviewer("Джейсон", "Стейтем")
first_reviewer.courses_attached += ["Python"]
second_reviewer = Reviewer("Джони", "Депп")
...
print(f"Средняя оценка у Иванова больше, чем у Петрова {second_student < first_student}")
print(f"Средняя оценка за лекции у Пушкина выше, чем у Васильева {second_lecturer > first_lecturer}\n")
